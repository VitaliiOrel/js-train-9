...
function findElementGreaterThan(arr, num) {
  ...
  return (arr.find(el => el > num)) || "Value not found"
...
console.log(findElementGreaterThan([1, 2, 3, 4, 5], 10)); // Виведе 4
...
function findLastEvenNumber(arr) {
  if (!Array.isArray(arr)) {return 'Вхідний параметр має бути масивом'}
    return (arr.findLast(el => el % 2 === 0)) || "Парних чисел не знайдено"
...
function getSubArrayAndConvertToString(arr, startIdx) {
  if (!Array.isArray(arr)) {return 'Вхідний параметр має бути масивом'}
    const arrCut = arr.slice(startIdx)
    const message = arrCut.join(" ")
  return message
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function getPositiveArrayValues(arr) {
  if (!Array.isArray(arr)) {return undefined}
    ...
    console.log(arrFiltered) // [ 1, 2 ] 
    console.log(arr) // виводе в консоль [ -2, -1, 0, 1, 2 ] - ПОЧАТКОВИЙ МАСИВ НЕ ЗМІНИВСЯ
    
    return arrFiltered //повертаємо масив глибокої логіки
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function flattenSquareAndFilter(arr, threshold) {
  if (!Array.isArray(arr)) {return 'Вхідний параметр має бути масивом'}
    ...
    const arrSquare = arrFlat.map(el => el ** 2)
    const filteredSquaredArray = arrSquare.filter(el => el > threshold)
...
function getLastElementsSumAndPrint(arr, n) {
  if (!Array.isArray(arr)) {return 'Вхідний параметр має бути масивом'}
    const arrSl = arr.slice(-n)
    const sumArrSl = arrSl.reduceRight((ac, el) => ac + el)
    arrSl.forEach(el => console.log(el))
    return sumArrSl
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function expandArrayByN(arr, n) {
  if (!Array.isArray(arr)) {return 'Вхідний параметр має бути масивом'}
    const expandedArray = arr.flatMap(el => {
        const arrRep = []
        for (let i = 1; i <= n; i++) {
            arrRep.push(el)
        }
...
function findDuplicateElements(arr) {
  if (!Array.isArray(arr)) {return 'Вхідний параметр має бути масивом'}
    let arrDup = arr.filter((el, inx) => {
        if (inx !== arr.indexOf(el)) {return el}
    })
    return arrDup
